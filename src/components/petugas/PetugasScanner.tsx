...
import { ArrowLeft, Flashlight, CheckCircle, AlertTriangle, CameraOff } from 'lucide-react';
...
interface PetugasScannerProps {
  onBack: () => void;
  onVerifyCode: (code: string) => Promise<VerificationResult> | VerificationResult;
}
...
export default function PetugasScanner({ onBack, onVerifyCode }: PetugasScannerProps) {
    ...
    const startScanner = async () => {
      ...
      try {
        await scanner.start(
          ...
          () => {
            // Ignore frame-level scan failures; the main success callback handles valid scans.
          }
        );
  ...
  const handleToggleFlash = async () => {
    ...
    try {
      ...
      setErrorMsg('Flash tidak tersedia pada kamera ini.');
    }
  };
          ...
          {!cameraReady && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/90 px-6 text-center">
              <div className="rounded-full bg-white/10 p-4 mb-3">
                <CameraOff size={28} className="text-white" />
              </div>
              <p className="text-sm font-semibold text-white">Menunggu akses kamera...</p>
              <p className="text-xs text-slate-300 mt-1">Buka kamera dari HP Anda untuk memindai QR.</p>
            </div>
          )}
        </div>
